...
>
> 我们用另一个 AI，花了两天、烧掉约 2 亿 token，把它从头到尾拆开看了一遍。
...
有意思的是——它自己大概率也是用 AI 写出来的（[证据在第 00 篇](docs/zh/00-项目概述.md#这些代码是人写的吗)）。

## 文档索引

| 编号 | 文档 | 一句话 |
|------|------|--------|
| 00 | [项目概述](docs/zh/00-项目概述.md) | Hermes 是什么、为什么这样设计、完整文件索引 |
| 01 | [架构分析](docs/zh/01-架构分析.md) | 跟着一条消息走完从输入到输出的全程 |
| 02 | [Agent 核心](docs/zh/02-agent核心.md) | 13,000 行的"上帝文件"（God Object）里藏了什么 |
| 03 | [工具系统](docs/zh/03-工具系统.md) | 66 个工具怎么注册、调度、审批和限流 |
| 04 | [技能系统](docs/zh/04-技能系统.md) | Agent 怎么从经验中学会做事 |
| 05 | [插件系统](docs/zh/05-插件系统.md) | 16 种 Hook（生命周期钩子）让 Python 代码接入 Agent 的任何阶段 |
| 06 | [Gateway 网关](docs/zh/06-gateway网关.md) | 一个进程同时服务 Telegram、Discord、Slack…… |
| 07 | [TUI 与 Web](docs/zh/07-tui与web.md) | 同一个 Agent 的三张脸（TUI 即终端用户界面） |
| 08 | [Cron 与外部协议](docs/zh/08-cron调度.md) | 让 Agent 自己醒来干活（Cron 定时调度），让编辑器调用 Agent（ACP 协议） |
| 09 | [环境与部署](docs/zh/09-环境与部署.md) | 从笔记本到 GPU 云的六种运行方式 |
| 10 | [批量运行与 RL](docs/zh/10-批量运行与rl.md) | Agent 不只是产品，也是训练数据工厂 |
| 11 | [工程实践](docs/zh/11-工程实践.md) | 587,000 行代码怎么测试、记日志、保安全 |
...
整个过程消耗约 **约 2 亿 token**，历时 **两天**。每个概念用 9 问模板分析（是什么 / 从哪来 / 在哪里 / 依赖谁 / 怎么工作 / 解决什么 / 为什么不用别的 / 出错了会怎样 / 哪里可以调），织入 Martin Fowler（软件工程领域知名技术作家）风格的叙事中。

方法论不是一开始就有的——它在工作过程中从用户的反馈里逐步长出来。完整的演进记录在[工作日志](docs/zh/99-工作日志.md)里。
...
联系方式：[X](https://x.com/hausewoods) | [Email](mailto:fanglin.me@gmail.com) | [GitHub](https://github.com/fang-lin)
