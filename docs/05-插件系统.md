...
用户消息进入
  ...
  │
  ├─→ pre_llm_call            (每轮 LLM 调用前，可注入上下文)
  │     │
  │     ├─→ pre_api_request    (每次 API 请求前)
  │     └─→ post_api_request   (每次 API 请求后)
  │
  ├─→ post_llm_call           (LLM 响应后)
  │
  ├─→ pre_tool_call           (工具调用前，⚠️ 可阻断！)
  │     │
  │     ├─→ transform_terminal_output  (terminal 输出变换)
  │     └─→ transform_tool_result      (工具结果变换)
  │
  ├─→ post_tool_call          (工具调用后)
  │
  ├─→ on_session_end          (会话结束)
  └─→ on_session_finalize     (最终清理)
```
...
Agent 启动
  │
  ├─ MemoryManager.add_provider()     最多接受一个外部 provider
  │
  ▼
每轮对话
  │
  ├─ build_system_prompt()            provider 贡献静态上下文
  ├─ prefetch_all(query)              按当前消息语义召回记忆
  ├─ sync_all(user_msg, asst_msg)     对话结束后写入/更新记忆
  │
  ├─ on_memory_write()                内置 MEMORY.md 变更时镜像到 provider
  ├─ on_pre_compress()                上下文压缩前提取关键信息防丢失
  └─ on_delegation()                  子 Agent 完成时通知父 provider

Agent 结束
  │
  └─ shutdown()                       flush 队列，逆序关闭
```
