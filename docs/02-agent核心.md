...
API 调用
  ...
  ├─ 成功 → 继续
  │
  ├─ 失败（非 429）→ jittered_backoff 等待 → 重试（最多 3 次）
  │                                            │
  │                                     全部失败 → 尝试 fallback
  │
  └─ 429（限流）
       ...
       │            （给 Provider 重置窗口时间）
       │
       └─ 第二次 → 标记凭证 exhausted → 切换下一个凭证
                    │
                    └─ 所有凭证 exhausted → 等冷却 / 尝试 fallback
```
...
```
            ...
            │                              │
            ▼                              ▼
    以下为示例配置：              备用也失败 → 继续链式尝试
    主力: Anthropic 的某模型       │
      ↓ fallback                  所有备用耗尽 → 返回错误
