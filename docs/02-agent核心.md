...
为什么不直接共享父 Agent 的所有工具？因为子代理运行在独立线程中，缺少 CLI 的交互上下文（没有 stdin、没有审批对话框）。如果子代理要执行一个危险操作（比如 `rm -rf`），父 Agent 的审批回调对它不可见。默认行为是 `_subagent_auto_deny`——自动拒绝所有需要审批的操作（`delegate_tool.py:55-67`）。对于 cron 任务或批量运行场景，可以配置 `delegation.subagent_auto_approve` 放开限制。
...
子代理还有运行时可观测性——`_active_subagents` 注册表（`delegate_tool.py:148`）让 TUI 界面可以显示当前有多少子代理在运行、各自在做什么，并支持单独中断某个子代理。
...
`agent/trajectory.py` 是一个很小的模块（57 行），位于 Agent 核心的输出侧——每次 `run_conversation()` 结束时，如果 `save_trajectories=True`（`run_agent.py:855`），就调用 `_save_trajectory()`（`run_agent.py:3723`）把完整的对话序列追加写入 JSONL 文件。成功的轨迹写入 `trajectory_samples.jsonl`，失败的写入 `failed_trajectories.jsonl`——失败的也保留，因为对研究者来说，"模型在哪里犯错"和"模型做对了什么"一样有价值。
