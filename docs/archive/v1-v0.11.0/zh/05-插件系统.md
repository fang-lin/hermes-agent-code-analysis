...
中文 | [English](../en/05-plugin-system.md)
...
**图：技能（自然语言/模型层）与插件（Python 代码/运行时层）在 Agent 核心中的位置关系**
...
**图：插件生命周期钩子的执行顺序——从会话开始到 LLM 调用到工具执行到会话结束**
...
**图：记忆插件（MemoryProvider）的完整生命周期——从 Agent 启动到每轮对话的构建、预取、同步、压缩到关闭**
