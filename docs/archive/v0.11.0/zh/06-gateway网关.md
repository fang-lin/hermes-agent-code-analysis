...
**图：GatewayRunner 的整体架构——多平台适配器汇入统一处理流，经 Agent 缓存执行后流式投递**
...
flowchart TD
    subgraph GR["GatewayRunner #40;run.py:620#41;"]
        ...
        SL["Slack Adapter"]
        MORE["..."]
        HM["handle_message#40;#41;\n用户授权检查\n命令解析 #40;/model, /new, /stop...#41;\n会话管理 #40;SessionStore#41;"]
        CACHE["Agent 缓存\nOrderedDict\n≤128 个实例\nLRU + 1h TTL"]
...
**图：平台适配器的必须实现方法与可选降级方法——新增平台只需实现 3 个方法即可运行**
...
**图：Cron 定时任务通过 Gateway 触发执行并将结果投递到指定平台的完整流程**

```mermaid
flowchart TD
    GW["GatewayRunner 后台\n每 60 秒调用 scheduler.tick#40;#41;"]
    TICK["scheduler.tick#40;#41;\n文件锁防并发 #40;.tick.lock#41;\n检查哪些 job 到期"]
    JOB["对每个到期 job：\n可选 pre-check script（判断是否需要唤醒 Agent）\n构建 prompt（注入 context、skills）\n实例化独立 AIAgent\n执行 agent.run_conversation#40;#41;\n投递结果"]
    DELIVER["投递目标解析 #40;_resolve_delivery_targets#41;"]
    LOCAL["'local' → 只写文件不发送"]
