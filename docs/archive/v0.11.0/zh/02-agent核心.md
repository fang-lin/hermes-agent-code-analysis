...
> **本章定位**：`run_agent.py` 单文件（13,293 行）+ `agent/` 子目录（52 文件，29,201 行）。这是 Agent 核心模块，系统的心脏。
> **关键类**：`AIAgent`（`run_agent.py:840`）——50+ 参数的有状态对话协调器。
...
**图：AIAgent 的三种创建者——CLI、Gateway（带缓存）、delegate_tool（子 Agent）**

```mermaid
flowchart TD
    CLI["CLI\n#40;cli.py#41;"]
    GW["Gateway\n#40;gateway/run.py#41;"]
    DT["delegate_tool\n#40;子 Agent 创建#41;"]
    AG["AIAgent"]

    CLI -->|"创建 1 个 AIAgent"| AG
    GW -->|"创建 ≤128 个 AIAgent #40;缓存#41;"| AG
    DT -->|"创建 N 个子 AIAgent"| AG
```
...
flowchart TD
    CALLER["调用者\nCLI / Gateway / 父 Agent"]
    AGENT["AIAgent\n─────────────────\n状态:\n· 会话历史 #40;messages list#41;\n· 系统提示 #40;cached#41;\n· 迭代预算 #40;共享#41;\n· 当前模型/provider\n· rate limit 状态"]
    LLM["LLM API\n─────────────\nAnthropic\nOpenAI\nBedrock\nGemini\n本地引擎\n...\n#40;Transport 抽象层#41;"]
    TOOLS["工具层\n─────────────\n66 个工具\n#40;terminal, file,\nweb, browser,\nskills, ...#41;\n包括子 Agent:\ndelegate_tool\n→ 创建新 AIAgent"]
    PERSIST["持久化\n─────────────\nSQLite 会话存储\nMEMORY.md\nUSER.md\ntrajectory.jsonl\ncheckpoint 快照"]

    CALLER -->|"run_conversation#40;#41; / interrupt#40;#41; / steer#40;#41;\nswitch_model#40;#41; / reset_session_state#40;#41; / close#40;#41;"| AGENT
    AGENT --> LLM
    AGENT --> TOOLS
    AGENT --> PERSIST
```
...
**图：AIAgent 从创建到关闭的生命周期状态——可被多次调用的长生命周期服务**

```mermaid
flowchart TD
    INIT["创建 #40;__init__#41;\n初始化客户端（OpenAI/Anthropic SDK）\n初始化工具集（discover + register）\n初始化记忆管理器\n初始化上下文压缩器"]
    ACTIVE["活跃（可被反复调用）"]
    RUN["run_conversation#40;#41; ← 可调用多次\n构建/复用系统提示\n核心循环（调模型 → 执行工具 → 循环）\n持久化会话"]
    OPS["interrupt#40;#41; ← 随时可从另一线程调用\nsteer#40;#41; ← 随时可从另一线程调用\nswitch_model#40;#41; ← 热切换，不中断会话\nreset_session_state#40;#41; ← 清零计数器，开始新会话"]
    CLOSE["关闭 #40;close#41;\n终止后台进程\n关闭沙箱环境\n关闭浏览器会话\n中断所有子 Agent\n关闭 HTTP 客户端"]
...
**图：AIAgent 构造函数 50+ 参数按功能分为四组（模型连接、回调接口、会话控制、Gateway 身份）**

```mermaid
graph TD
    subgraph INIT["AIAgent.__init__#40;#41;"]
        MODEL["模型连接\nbase_url\napi_key\nprovider\napi_mode\nmodel\nfallback_model"]
        CALLBACK["回调接口\ntool_*_callback\nthinking_cb\nreasoning_cb\nclarify_cb\nstep_cb\nstatus_cb"]
        SESSION["会话控制\nsession_id\nmax_iterations\niteration_budget\nsave_trajectories\ncheckpoints\nprefill_messages"]
        GW["Gateway 身份\nplatform #40;cli/telegram/discord/...#41;\nuser_id, user_name, chat_id\ngateway_session_key"]
    end
```
...
**图：Prompt Caching 的 "system_and_3" 策略——系统提示占一个断点，最近三条消息各占一个**

```mermaid
flowchart TD
    SP["system prompt\n← breakpoint 1（最稳定，几乎不变）"]
...
- **系统提示只构建一次**（`run_agent.py:9814`），会话内不重建（除非上下文压缩改变了消息结构）。动态信息（如记忆检索结果）注入到**用户消息**而非系统提示——这就是 [01-架构分析](01-架构分析.md) 中介绍的"双注入"策略的另一个动机。
...
基础延迟 5 秒，每次翻倍，上限 120 秒。更简单的做法是纯指数退避（不加抖动），但这在多会话场景下会产生"惊群"效应（thundering herd）——假设 Gateway 同时服务 50 个用户，Provider 返回 429 后所有会话同时等 5 秒再重试，50 个请求同时砸过去，再次限流。Jitter 给每个会话的重试时间加一个随机偏移（`[0, 0.5 × delay]`），让请求在时间上分散开。
...
**图：API 调用失败的分层重试策略——普通错误指数退避，429 限流触发凭证轮转**
...
**图：Credential Pool 的持有关系——由 CLI/Gateway 创建，Agent 作为消费者从池中选取可用凭证**
...
flowchart LR
    ...
    subgraph POOL["Credential Pool"]
        CA["凭证 A #40;ok#41;"]
        CB["凭证 B #40;exhausted#41;"]
        CC["凭证 C #40;refreshing#41;"]
    end
    LLM["LLM API\nAnthropic\nOpenAI\n..."]

    CLI -->|创建| POOL
    CA -->|选中| LLM
    POOL -.->|"AIAgent 使用（不拥有）"| LLM
```
...
**图：Credential Pool 中单个凭证的三种状态转换（ok → exhausted/refreshing → ok）**
...
stateDiagram-v2
    ...
    ok --> refreshing : token 接近过期
    refreshing --> ok : 成功（新 token）
    refreshing --> exhausted : 失败
```

- **ok** — 可用，正常选取
- **exhausted** — 被限流（429）或余额不足（402），进入冷却期（默认 1 小时，`credential_pool.py:73`）。冷却结束后自动恢复
- **refreshing** — OAuth token 接近过期，正在用 refresh_token 换新的（`credential_pool.py:575-735`）

如果所有凭证同时 exhausted 了怎么办？Agent 会退回到正常的重试退避逻辑——等待冷却时间过去。如果配置了 fallback_model（下一节会讲），还可以自动切换到另一个 Provider 的模型，用完全不同的凭证继续工作。

解决了"用什么凭证调 API"和"调用失败怎么恢复"之后，下一个问题是——响应回来了，怎么交付给用户？
...
一个设计选择：**工具调用轮次完全静默**。当模型响应包含工具调用时，流式回调不触发——用户不会看到"我要搜索一下网页"这样的中间文字逐字蹦出来。只有最终的纯文本回复才会流式传输。替代方案是也流式展示工具调用意图（比如 "Searching for..."），但 Hermes 选择用工具进度回调（`tool_progress_callback`）和 spinner 动画来替代，保持输出区的干净。
...
**图：Fallback Chain 跨 Provider 自动降级——重试耗尽后链式尝试备用模型，主力恢复后自动切回**

```mermaid
flowchart TD
    FAIL["API 调用失败\n重试 + 凭证轮转均耗尽"]
    HASFB{配置了 fallback_model？}
    ERR["返回错误给用户（明确失败）"]
    FB["_try_activate_fallback#40;#41;"]
    CHAIN["备用也失败 → 继续链式尝试\n所有备用耗尽 → 返回错误"]
    CHAIN2["示例配置:\n主力: Anthropic 的某模型\n  ↓ fallback\n备用 1: OpenRouter 的某模型\n  ↓ fallback\n备用 2: OpenAI 的某模型\n  ↓ 成功，继续对话"]
...
## 子 Agent：当一个 Agent 忙不过来

Fallback Chain 是纵向降级——模型不行了换一个。子 Agent是横向分拆——任务太大了，拆成几块并行处理。两者都是 Agent 核心的弹性机制，但方向不同。

子 Agent的设计源自一个实际场景：用户说"分析这三个文件然后写一份报告"。分析三个文件可以并行，但写报告要等分析完成。如果单个 Agent 串行做，时间是三倍；如果能 spawn 三个子 Agent 并行分析，再汇总写报告，效率高得多。
...
**图：父 Agent 并行分拆三个子 Agent 处理独立子任务，汇总后继续执行**

```mermaid
flowchart TD
    PARENT["父 Agent #40;depth=0#41;\n模型输出: delegate_task#40;tasks=[...]#41;"]
    A["子 Agent A #40;depth=1#41;\n分析文件 X"]
    B["子 Agent B #40;depth=1#41;\n分析文件 Y"]
    C["子 Agent C #40;depth=1#41;\n分析文件 Z"]
...
子 Agent运行在 `ThreadPoolExecutor` 的工作线程中，默认最多 3 个并发（`delegate_tool.py:128`，`_DEFAULT_MAX_CONCURRENT_CHILDREN = 3`）。它们共享父 Agent 的 `iteration_budget`——如果父预算 90 次迭代，三个子 Agent 加起来也只能用这 90 次中剩下的部分。这防止了子 Agent失控消耗资源。

安全隔离是子 Agent设计的核心考量。子 Agent 不是父的完整克隆——它的工具集是父的**子集**（取交集，`delegate_tool.py:891-930`），并且有五个工具被强制禁用（`delegate_tool.py:41-49`）：

- `delegate_task` — 防止无限递归（除非子 Agent的 role 是 `orchestrator`）
- `clarify` — 子 Agent在后台线程，没有 stdin，无法交互
- `memory` — 避免多个子 Agent并发写 MEMORY.md 导致冲突
- `send_message` — 防止子 Agent擅自给用户发消息
- `execute_code` — 强制逐步推理，不走捷径

为什么不直接共享父 Agent 的所有工具？因为子 Agent运行在独立线程中，缺少 CLI 的交互上下文（没有 stdin、没有审批对话框）。如果子 Agent要执行一个危险操作（比如 `rm -rf`），父 Agent 的审批回调对它不可见。默认行为是 `_subagent_auto_deny`（`delegate_tool.py:69`）——自动拒绝所有需要审批的操作。对于 cron 任务或批量运行场景，可以配置 `delegation.subagent_auto_approve` 放开限制。

嵌套深度默认限制为 1 层（`delegate_tool.py:129`，`MAX_DEPTH = 1`）——父 Agent spawn 子 Agent，但子 Agent 不能再 spawn 孙 Agent。这个限制可以通过 `config.yaml` 的 `delegation.max_spawn_depth` 放宽到最多 3 层（`delegate_tool.py:133`）。如果需要更深的嵌套，可以给子 Agent设置 `role: "orchestrator"`，它会保留 `delegate_task` 工具并能继续分拆任务。

子 Agent还有运行时可观测性——`_active_subagents` 注册表（`delegate_tool.py:151`）让 TUI 界面可以显示当前有多少子 Agent在运行、各自在做什么，并支持单独中断某个子 Agent。
...
但核心循环——"调用模型 → 解析响应 → 执行工具 → 循环"——以及所有这些子系统之间的协调逻辑，仍然需要一个地方来编排。这就是 `run_agent.py` 的角色：**不是做所有事情的类，而是把所有子系统串在一起的胶水**。只是当系统足够复杂时，胶水本身也需要 13,000 行——编排 10 个子系统的交互、处理它们之间的时序依赖和错误传播，这本身就是一个与实现子系统同等复杂的任务。
