...
中文 | [English](../en/07-tui-and-web.md)
...
**图：Hermes 的三种用户界面——prompt_toolkit TUI、React/Ink TUI、Web Dashboard，共享同一 Agent 核心**

```mermaid
graph LR
    subgraph F1["面孔 1: prompt_toolkit TUI\n纯 Python，启动最快"]
        CLI["cli.py"] --> AG1["AIAgent"]
    end
    subgraph F2["面孔 2: React/Ink TUI\n更丰富的 UI，但多一层进程间通信"]
        UITUI["ui-tui/ #40;Node.js#41;"] -->|JSON-RPC| TUIGW["tui_gateway/"] --> AG2["AIAgent"]
    end
    subgraph F3["面孔 3: Web Dashboard\n浏览器访问，最灵活但链路最长"]
...
**图：React/Ink TUI 通过 stdio JSON-RPC 桥接 Python Agent 的跨进程通信架构**
...
**图：Web Dashboard 的 Chat 嵌入链路——浏览器 xterm.js 经 WebSocket、PTY、Ink TUI 最终连接 AIAgent**

```mermaid
flowchart TD
    XTERM["浏览器 xterm.js #40;WebGL 渲染#41;"]
