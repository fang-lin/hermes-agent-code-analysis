...
一个日常可见但容易被当成"魔法"的输入语法：消息里写 `@file:path`、`@folder:src/`、`@diff`、`@staged` 会被**就地展开**成对应的文件内容、目录树、或 git 差异，注入这一条消息（`agent/context_references.py`，598 行；展开发生在发送前、按上下文长度预算裁剪）。它是"把上下文喂给 Agent"的轻量手段——不用先 `cat` 再粘贴，直接 `@diff 帮我 review` 即可。CLI 和消息平台通用。
