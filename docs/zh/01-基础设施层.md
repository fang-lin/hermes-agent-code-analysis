...
> **本章定位**：`hermes_cli/` 目录（97 个 .py 文件，约 100,800 行，含 proxy/ 子模块）——CLI 子命令、配置系统、认证系统、插件管理、Profile 隔离、网关服务管理。
...
这些问题的答案都在 `hermes_cli/` 里。它是 hermes-agent 的**控制平面**——不直接参与对话，但决定了对话在什么环境下发生。它有约 100,800 行代码（含 proxy/ 子模块），是整个项目最大的模块，比 Agent 核心（63,679 行）和网关层（80,025 行）都大。
...
**场景三：安装 Gateway 为系统服务。** `hermes gateway install`（子命令分发入口 `gateway_command()`，`gateway.py:5006`）根据操作系统自动生成 systemd unit 文件（Linux）或 launchd plist（macOS），让网关随系统启动——实际生成逻辑在 `systemd_install()`（`gateway.py:2440`）和 `launchd_install()`（`gateway.py:2900`）。
...
**model_switch.py**（1,799 行）处理模型切换——当用户输入 `/model sonnet` 时，它需要把别名解析为完整的 `(provider, model_id)`。`resolve_alias()`（`model_switch.py:450`）会依次查找：`DIRECT_ALIASES`（内置别名 + `config.yaml` 的 `model_aliases:` 用户自定义别名）→ 内置家族别名 `MODEL_ALIASES`（如 `sonnet` 映射到具体版本）→ Provider 模型目录的前缀匹配（`startswith`，而非编辑距离式的模糊匹配）。
...
**setup.py**（3,607 行）是交互式配置向导的编排器，`SETUP_SECTIONS` 定义六个步骤（选 Provider 和模型 → 配 TTS → 选终端后端 → 配消息平台 → 配工具集 → 设 Agent 参数）。它本身不实现任何配置逻辑——每个步骤都委托给专门的模块。
...
`kanban.py`（2,762 行）+ `kanban_db.py`（6,579 行）是一个完整的多 Agent 协作系统——看板、DAG 任务依赖（`_would_cycle()`，`kanban_db.py:1906`）、乐观锁认领（`host:pid` 标识 + TTL，`_claimer_id()` 返回 `f"{host}:{pid}"`，`kanban_db.py:1504`）、断路器（`consecutive_failures` + `max_retries`）。每个看板是一个独立的 SQLite 文件，备份和归档只需复制 .db 文件。
...
插件不是默认全部加载的。但要分两类看：**内置的 backend / platform 类插件**（`manifest.source == "bundled"` 且 `kind in {"backend", "platform"}`，如 IRC 网关适配器）随 hermes 自动加载，做到开箱即用；**其余插件**（standalone、用户安装的 backend、entry-point 插件）才受 `plugins.enabled` 白名单控制——只有显式列出才会被加载。这是一个安全决策：第三方插件可以注册任意工具和钩子，无限制加载会带来安全风险。`migrate_config()` 在用户升级时会自动把已有的用户插件加入白名单，避免升级后插件突然消失。`migrate_config()` 在用户升级时会自动把已有的用户插件加入白名单，避免升级后插件突然消失。
