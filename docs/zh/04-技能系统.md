...
| 后台审查线程失败 | 日志 `WARNING: Background memory/skill review failed`（stderr / agent.log）。stdout 被重定向到 devnull，用户界面看不到错误 |
...
- **`action="patch"`** — 用 unified diff 格式做局部更新，最外科手术式的修改方式（以 `polymarket` 技能的 API 端点列表需要补充一个新字段为例）。patch 使用 fuzzy match 定位，容忍小范围的上下文变化
...
**使用遥测**：`~/.hermes/skills/.usage.json` 记录每个技能的使用数据（`skill_usage.py:307-320`）：`view_count`（被浏览次数）、`use_count`（被加载次数）、`patch_count`（被修改次数）、`last_used_at`（最后使用时间）、`created_by`（来源标记——`"agent"` 表示后台 review 创建，`None` 表示其他来源）。`skill_view` 成功时 bump `view_count`，`skill_manage` 操作时 bump 对应计数器。

**Curator 触发条件**（`curator.py:56-59`）：不使用 cron——当 Agent 空闲且距上次运行超过 `interval_hours`（默认 7 天）时，`maybe_run_curator()` 触发。要求至少空闲 `min_idle_hours`（默认 2 小时）。Curator 使用辅助模型（auxiliary client），不影响主会话的 prompt cache。
