...
| 技能修改后没生效 | 磁盘快照（mtime 检测）通常自动失效，但**进程内 LRU 不会**——重启 Agent 或调用 `clear_skills_system_prompt_cache()` |
| 技能没有自动创建 | 检查四个触发条件：`creation_nudge_interval > 0`、工具迭代次数够、`skill_manage` 工具集已启用、对话正常完成（非中断）。入口函数 `build_skills_system_prompt()`（`prompt_builder.py:997`） |
...
| `conditions` | 条件激活——四个子字段控制技能何时出现（见下方详解） |

**conditions 条件激活的四种子类型**（`skill_utils.py:341-354`）：
...
以 `skills/productivity/maps/` 为例，它声明 `requires_toolsets: [terminal]`——在没有终端工具集的环境（以某些 Gateway 平台为例）中，这个技能会被静默过滤，不出现在系统提示中。
...
**来源追踪**：`skill_provenance.py` 用 ContextVar 区分"后台 review 自动创建"和"前台用户指令创建"。只有 `is_background_review()` 为 True 时，`skill_manage(action="create")` 才会调用 `mark_agent_created()`（`skill_manager_tool.py:884-885`），技能才会被 Curator 管理——这就是为什么用户手动指示 Agent 创建的技能不会被 Curator 自动清理。
...
- **pinned** — 手动标记，绕过所有自动状态转换

**使用遥测**：`~/.hermes/skills/.usage.json` 记录每个技能的使用数据（`skill_usage.py:63-64`）：`view_count`（被浏览次数）、`use_count`（被加载次数）、`patch_count`（被修改次数）、`last_used_at`（最后使用时间）、`agent_created`（是否由后台 review 创建）。`skill_view` 成功时 bump `view_count`，`skill_manage` 操作时 bump 对应计数器。
