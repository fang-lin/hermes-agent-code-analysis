...
**图：Cron 数据目录结构——jobs.json 存定义、.tick.lock 防并发、output/ 归档每次执行结果**
...
**图：ACP 服务端架构——编辑器通过 stdio JSON-RPC 连接 Hermes，每个编辑器会话对应一个独立 AIAgent**

```mermaid
flowchart TD
    EDITOR["编辑器 #40;Zed/VS Code#41;"]
...
到此为止，我们已经覆盖了 Hermes 的所有主要子系统。接下来的 **09-ACP 适配** 将被合并到本章（已覆盖）。**09-环境与部署** 会聚焦运维——Docker 构建、终端后端配置、多 Profile 管理。
