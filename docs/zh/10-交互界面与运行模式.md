...
**这就是「非阻塞输入」的根因**——主线程从不被 Agent 占用，所以你随时能打字。还有第三个 daemon 线程跑 `spinner_loop`（函数定义在 `cli.py:13983`，于 13999 起为 daemon 线程）专门驱动那个卖萌的 spinner 动画。
...
Web Dashboard（`hermes web`，FastAPI，默认 `http://127.0.0.1:9119`，`web_server.py:8`）走得更远：它的 Chat 页面在浏览器里嵌入完整终端体验——浏览器 xterm.js ←WebSocket← FastAPI PtyBridge ←PTY← 真实的 Ink TUI ←→ tui_gateway ←→ AIAgent。也就是说，「一个 gateway，多个客户端」——TUI、Web Dashboard、消息平台可以同时连同一个长驻 gateway，共享状态（`tui.md` 的「Attaching to a running gateway」）。Web Dashboard 的安全机制：每次启动生成 ephemeral session token（生成在 `web_server.py:87`，`_SESSION_TOKEN = secrets.token_urlsafe(32)`；`_has_valid_session_token`:125 与 `_require_token`:146 负责校验，401 拒绝）、CORS 仅 localhost、Host 头校验防 DNS rebinding（`web_server.py:151-177`）。
...
把 Hermes 嵌进脚本、开并行 worktree、挂 tmux 远程跑——这些用法都有一个隐含前提：你得打字。但开车时、白板前、或手里正拿着工具时，打字并不总是选项。Hermes 的**语音交互**让你用说话代替打字，也让 Agent 用说话回复，甚至加入 Discord 语音频道实时参与讨论。这套能力横跨 CLI 和消息平台，核心代码在 `tools/voice_mode.py`（1,129 行，录音/播放）、`tools/transcription_tools.py`（语音转文字 STT）、`tools/tts_tool.py`（文字转语音 TTS）。
...
**STT/TTS 都是多 provider 可切换的**（`config.yaml` 的 `stt:`/`tts:` 段）。STT 默认按 **local（faster-whisper，免费免 key，约 150MB 模型首次自动下载）→ Groq → OpenAI → xAI** 的优先级自动回退（`transcription_tools.py:303`）；TTS 有近十种内置 provider——以 `edge`（Microsoft Edge TTS，免费免 key，默认）、`elevenlabs`（付费高质量）、`openai`、`neutts`（本地）为例，另有 `minimax`、`gemini`、`xai`、`kittentts` 等及自定义 command provider（完整列表见官方 TTS 文档）。所以零 API key 也能跑——本地 Whisper + Edge TTS 全程不花钱。
