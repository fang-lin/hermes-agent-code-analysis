...
**NFS 兼容性降级**：WAL 模式依赖共享内存，在 NFS/SMB 网络文件系统上会失败。Hermes 检测到失败时降级到 `journal_mode=DELETE`（并发性下降但可用），并且对每个数据库每个进程只警告一次，不刷屏。配套有一条**诊断链路**：DB 初始化失败的原因会被记到模块级的 `_last_init_error`，当 `/resume`、`/title`、`/branch` 这些命令发现 `state.db` 不可用时，会把这个原因（通常是 NFS/SMB 的 "locking protocol" 错误）格式化进给用户的提示里（`/history` 例外——它在没有 DB 时只静默打印"暂无历史"，不带诊断原因）——而不是甩一句干巴巴的「Session database not available」。把「底层为什么挂」一直传到「用户看到的错误」，是这个项目对可排查性的一贯讲究。
...
### 测试：2,017 个文件，但真正的功夫在隔离

`tests/` 有 2,017 个 `.py` 文件（其中 `test_*.py` 1,960 个，其余是 conftest 等夹具）。但数量不是重点，**测试隔离**才是。`tests/conftest.py` 开头就声明了一组「hermetic（密封）不变量」——hermetic 字面意思是气密，在测试里指每个测试都跑在一个与外部隔绝的气泡里：不被环境污染、跑完不留痕迹。强制规则如下：
...
更狠的是执行策略：**每个测试文件起一个独立子进程**（独立 Python 解释器），而不是用 `pytest-xdist` 的持久 worker。为什么？因为持久 worker 会跨文件泄漏状态——模块级字典、ContextVar、各种缓存，一个文件污染了下一个文件就偶发失败。文件级子进程隔离从根上消除了这类泄漏。CI 里用可配的分片矩阵并行跑（`tests.yml`，`slice_count` 默认 **8**，v0.18 从硬编码 6 路改为 workflow_call 可复用参数；依赖安装用 `uv sync --locked`），按缓存的历史时长分片，兼顾隔离和速度。子进程隔离的代价有实测注脚（`scripts/run_tests_parallel.py` 头注释）：若按**每个测试**起子进程，约 1.7 万个测试 × 250ms ≈ 70 分钟 CPU；改成**每个文件**起子进程（约 850 个文件 × 250ms）压到约 3.5 分钟——注意这个收益来自"放弃 xdist、自研 per-file 子进程池"，不是 xdist（xdist 的持久 worker 恰恰是要消除的跨文件泄漏源）。

隔离还有第二道防线：一个 `autouse` 的 `_live_system_guard`（fixture 定义在 `conftest.py:547-548`）。它拦截 `os.kill`、`os.killpg`、`subprocess.*`、`os.system` 等原语，防止某个测试忘了 mock 就真的向测试进程子树**之外**的进程发信号——尤其是开发者本机正在跑的 `hermes-gateway`。少了这道防线，一个写漏的测试可能在跑测试时把你的 gateway 给 SIGTERM 了。从测试文件名能看出关注点的优先级：`test_concurrent_*`（并发竞争）、`test_*_wal_*`（WAL/NFS 降级）、`test_*_injection*`（注入）、`test_*_compression*`（会话切分不丢消息）——安全和并发是一等公民。

### 供应链硬化：防的是真实发生过的投毒

这是 v0.14 立下、延续至今的一个很「时代性」的部分。`pyproject.toml` 里的直接依赖**绝大多数精确钉版** `==X.Y.Z`（30 个依赖里 6 条用范围：urllib3/fastapi/uvicorn/python-multipart 加互斥的 ptyprocess/pywinpty 一对——若把这对 PTY 依赖算作一个逻辑项即"5 个"，与第 00 章口径一致；注释的"全部钉版"口号与现实有轻微漂移）。注释（`pyproject.toml:25-38`）讲了原因，而且是个真实事件：
...
- **供应链顾问扫描（被动）**（`hermes_cli/security_advisories.py`）：每次启动用 stdlib 查一遍 venv 里的包有没有命中**内置的已知投毒版本目录**，命中就在 banner/`hermes doctor`/gateway 日志告警。旧告警**故意不删**——注释给的理由是：跑着旧版 Hermes、依赖仍钉在被投毒固定版本上的用户，需要继续收到告警（被取代的条目用 `superseded_by` 标记，而非删除）。
- **供应链审计（主动/按需）**（`hermes_cli/security_audit.py`，`hermes security audit` 子命令）：与上面的被动扫描是两条不同机制——它按需把 venv 依赖 + 插件依赖 + MCP 服务器版本打包发给 **OSV.dev**（`api.osv.dev/v1/querybatch`，每批上限 1000）实时查已知漏洞，而不是只对内置的投毒目录做本地匹配。被动扫描管"已知的几个投毒事件"，主动审计管"全量 CVE 面"。
...
tests/                 — 2,017 文件，conftest hermetic 隔离 + per-file 子进程
