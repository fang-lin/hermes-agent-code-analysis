...
2. **短超时 + 应用层重试**：SQLite 默认锁等待 30 秒；Hermes 把它压到 **1 秒**，然后在应用层自己重试——最多 15 次（`_WRITE_MAX_RETRIES=15`），每次等一个 **20–150ms 的随机抖动**（`_WRITE_RETRY_MIN_S=0.020`/`_MAX_S=0.150`）。
...
v13 起，Hermes 改用**声明式列协调**：你只要在 `SCHEMA_SQL`（建表语句）里加一个列，下次启动时它会自动出现在数据库里——不需要写任何迁移代码。机制是（`hermes_state.py:466-550`）：`_parse_schema_columns()` 用一个**内存 SQLite** 解析建表语句，得到「每张表应该有哪些列」——为什么不用正则？因为 DDL 里有带逗号的 DEFAULT 表达式、行内 REFERENCES、CHECK 约束，正则解析全是边界 bug；让 SQLite 自己解析、再 `PRAGMA table_info` 读列，零正则边界。
...
`utils.py` 只有 361 行，但它的 `atomic_json_write()`（`utils.py:85`）和 `atomic_yaml_write()` 被整个系统依赖——Cron 的 `jobs.json`、Profile 的 active_profile、批量运行的 checkpoint、OAuth 凭证、用户配置，全靠它写。
