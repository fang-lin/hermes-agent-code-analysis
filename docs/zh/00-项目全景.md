...
Hermes 的选择是**完全不锁定模型**。它内置支持 30+ 种 Provider（注册表 `CANONICAL_PROVIDERS` 静态列表就有 35 条，`cli-config.yaml.example` 的注释段以其中约 22 个常用项为例）：OpenRouter（300+ 模型）、Anthropic、OpenAI、Nous Portal、Gemini、NVIDIA NIM、小米 MiMo、Kimi、MiniMax、Hugging Face、Arcee、Ollama Cloud、KiloCode……甚至支持本地运行的 Ollama、LM Studio、vLLM、llama.cpp。用户可以随时用 `hermes model` 切换，不需要改代码。
...
一切从 `cli.py` 的主循环开始。`HermesCLI` 类用 `prompt_toolkit` 构建了一个交互式 REPL。用户输入通过 TextArea 捕获（主线程的 prompt_toolkit UI 事件循环负责），放入一个线程安全的队列。一个**后台 daemon 线程**（`process_loop`）以 100ms 间隔轮询这个队列，拿到输入后判断是斜杠命令还是普通消息，并在该线程中启动 Agent 调用——主线程则始终专注维护 REPL 界面，不被 Agent 执行阻塞。
...
当模型响应包含工具调用时，`_execute_tool_calls()`（`run_agent.py:3964`）会判断这批调用是否可以并行：只读工具（以 `read_file`、`web_search` 为例）总是可以走并行路径；有副作用的工具则更谨慎——以文件读写为例，只有当它们的目标路径互不重叠时才并行，路径可能冲突就退回串行。完全串行太慢，完全并行可能有竞态——按副作用和路径重叠判定是一个务实的折中。
