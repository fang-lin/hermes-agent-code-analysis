...
| `toolset_distributions.py` | 文件 | 364 | 数据生成用工具集概率分布（→ [11](10-批量运行与rl.md)） |
| `mcp_serve.py` | 文件 | 867 | MCP 服务端，暴露消息网关能力（→ [08](08-cron调度.md)） |
| `hermes_constants.py` | 文件 | 295 | 共享常量（HERMES_HOME 等） |
| `hermes_state.py` | 文件 | 2,094 | SQLite 会话存储（→ [12](11-工程实践.md)） |
| `hermes_logging.py` | 文件 | 389 | 三路日志分发（→ [12](11-工程实践.md)） |
| `hermes_time.py` | 文件 | 104 | 时间工具函数 |
| `utils.py` | 文件 | 271 | 原子写入等通用工具（→ [12](11-工程实践.md)） |
...
| `environments/` | 30 | 7,306 | RL 训练环境（→ [11](10-批量运行与rl.md)） |
| `tests/` | 826 | 285,239 | 测试套件（→ [12](11-工程实践.md)） |
...
| 路径 | 行数 | 说明 |
|------|------|------|
| `batch_runner.py` | 1,287 | 批量轨迹生成（→ [11](10-批量运行与rl.md)） |
| `trajectory_compressor.py` | 1,508 | 轨迹压缩（→ [11](10-批量运行与rl.md)） |
| `rl_cli.py` | 446 | RL 训练入口（→ [11](10-批量运行与rl.md)） |
...
| 路径 | 说明 |
|------|------|
| `Dockerfile` | Docker 三阶段构建（→ [10](09-环境与部署.md)） |
| `docker-compose.yml` | gateway + dashboard 双服务 |
| `setup-hermes.sh` | 一键安装脚本（→ [10](09-环境与部署.md)） |
...
| `CONTRIBUTING.md` | 贡献指南（→ [12](11-工程实践.md)） |
| `SECURITY.md` | 安全政策（→ [12](11-工程实践.md)） |
