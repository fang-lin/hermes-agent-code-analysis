...
> **关键目录**：`plugins/memory/`（8 个记忆 Provider，11,647 行）、`plugins/model-providers/`（29 个子目录，1,199 行）、`plugins/platforms/`（7 个消息平台，15,326 行）。
...
`plugins/model-providers/` 包含 29 个子目录（1,199 行），合计注册出 30 余个 Provider（多数一目录注册一个，个别如 minimax/gemini 注册多个）。这是 01 章提到的 `PROVIDER_REGISTRY` 自动扩展机制（`auth.py:459`）的具体实现——每个插件在加载时把自己的 `ProviderConfig` 注册到认证系统。
...
`plugins/observability/langfuse/` 实现了对 Langfuse 的集成——追踪 Hermes 的对话、LLM 调用和工具使用。它通过 `pre/post_api_request`、`pre/post_llm_call`、`pre/post_tool_call` 六个钩子采集数据，不侵入 Agent 核心逻辑。
...
- **context_engine**（219 行）——上下文引擎插件的发现入口（目录下目前只有 `__init__.py`）；具体引擎（如 `load_context_engine("lcm")` docstring 示例里的 LCM）按需以独立插件形式提供，并非已内置在此目录
...
核心平台（Telegram、Slack 等 16 个）直接在 `gateway/platforms/` 下，不需要插件机制。插件平台（Discord、Teams 等 7 个）走插件路径有两个原因：(1) 它们有额外的 pip 依赖（以 `discord.py` 为例），放进插件目录隔离、按需安装比塞进核心依赖更干净（注：`pip_dependencies` 字段实际用于记忆插件；平台插件的 `plugin.yaml` 用 `requires_env` 声明所需环境变量）；(2) 它们是后来添加的，插件路径让添加新平台不需要修改核心 gateway 代码。
