...
图像生成后端通过 `ctx.register_image_gen_provider()` 注册，v0.18 扩到 6 个：OpenAI（gpt-image-2）、FAL、xAI、OpenAI-Codex，新增 **Krea** 和 **OpenRouter**（合计 2,987 行）。选择算法（`get_active_provider()`，`agent/image_gen_registry.py:75`）有个反直觉设计：**显式设置了 `image_gen.provider` 就直接用它、不做可用性过滤**——让下游报出精确的"X_API_KEY 未设置"，而不是静默换后端；未设置时才走可用性过滤的隐式回退（单个可用则用之 → 历史遗留的 FAL 优先 → 都没有则提示去 `hermes tools` 配置）。"图片用错了后端"先看 provider 是否显式设置。视频生成（fal/xai，1,545 行）通过 `ctx.register_video_gen_provider()` 注册，模式相同。
...
- **nemo_relay**（v0.18 新增）——对接 NVIDIA NeMo 侧中继，**双模式**运行：默认 observe_only——15 种钩子（六个 API/LLM/工具对 + 会话四件套 + 审批对 + subagent 起止 + api_request_error）纯观察；开启 adaptive（经独立的 `plugins.toml` 配置通道，`HERMES_NEMO_RELAY_PLUGINS_TOML`）后，它注册的两个**执行中间件**（`register_middleware("llm_execution"/"tool_execution")`，`__init__.py:420-421`）会把 LLM/工具调用的实际执行包进 Relay 的 `llm.execute`/`tools.execute`（`managed_llm_enabled()`，`:275-286` 门控）——此时对应的观察钩子被短路防止重复记录。它还有 ATOF/ATIF 两种轨迹落盘导出。这是仓库里唯一同时用上钩子和中间件（07 章）两套扩展面的生产插件。一个已知边界：adaptive 模式下它靠字符串前缀从 Relay 的包装异常里还原真实下游错误——Relay 改格式解包会静默失效，届时报错会显得笼统
...
面向 Microsoft Teams 会议——但与 google_meet 不同，它只注册 CLI 命令（`hermes teams-pipeline`），不注册 Agent 工具。工作流程：获取会议录制 → 转录 → 辅助 LLM 生成摘要（含关键决策、行动项、风险评估）→ 写入多个 sink（Notion 数据库、Linear team、Teams channel）。LLM 摘要失败时有启发式 fallback；sink 写入按 `sink:meeting_id` 做幂等去重（重复触发不重复写），且各 sink 独立 try/except——一个失败不拖累其余。
...
这套框架分两层：**框架层**在 `hermes_cli/dashboard_auth/`（routes/middleware/cookies/ws_tickets 等）——统一承载"登录发起 → 回调校验 → session cookie → 中间件校验 → 刷新 → WebSocket 票据 → 登出"的完整生命周期；**方案层**才是这四个插件，各自只实现"完成登录"那一步。契约是 `DashboardAuthProvider` ABC（`hermes_cli/dashboard_auth/base.py`）：`Session`（9 字段 frozen dataclass）、`TokenPrincipal`（服务间 token 主体）、`LoginStart`（OAuth 第一跳）加四种映射到不同 HTTP 状态码的专用异常。
