...
`plugins/google_meet/` 实现了 Google Meet 会议转录集成。插件注册了 5 个工具：`meet_join`（加入会议）、`meet_status`（查询会议状态）、`meet_transcript`（读取转录内容）、`meet_leave`（离开会议）、`meet_say`（在会议中发言）。v1 模式通过 Playwright 爬取 Meet 的 live captions 到 transcript 文件，Agent 通过 `meet_transcript` 工具读取该文件——不是实时注入，而是文件中转模式。
