...
| Dashboard 登录行为不解 | 认证门**只要绑定非回环地址就启用**（`--insecure` 已不再能绕过，见架构节）；provider 配置不全时 fail-closed 拒绝登录（查 LAST_SKIP_REASON） |
...
两个使用侧关键分支：**认证门只要 Dashboard 绑定非回环地址就启用**（权威逻辑在 `web_server.py:389` 的 `should_require_auth()`：loopback→不认证、非 loopback→必须认证）——本地 127.0.0.1 跑永远不会看到登录页。注意一个已收紧的行为：`--insecure`（`allow_public`）**曾经**能关掉认证门，但 June 2026 的 `hermes-0day` MCP 持久化事件后已改为**被忽略**——非回环绑定一律要求认证 provider，`--insecure` 只为兼容旧启动脚本保留、不再生效（`plugins.py:678` 那段仍写"未带 --insecure"的 docstring 是尚未同步的过期注释）。provider 加载了但配置不全拒绝注册时走 **fail-closed**（全部登录被拒，原因记在 `LAST_SKIP_REASON`）——"上线后登录不了"先查这两处。provider 加载了但配置不全拒绝注册时走 **fail-closed**（全部登录被拒，原因记在 `LAST_SKIP_REASON`）——"上线后登录不了"先查这两处。插件经 `ctx.register_dashboard_auth_provider()` 注册，做成插件而不是写死在 web_server 里，第三方部署可以带自己的 SSO。
