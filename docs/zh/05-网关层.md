...
- `gateway/slash_commands.py`（4,624 行）——42 个网关斜杠命令 handler
...
**❺ 命令解析**。检查消息是否是斜杠命令（`/model`、`/new`、`/stop`、`/compress` 等，42 个 handler 在 `gateway/slash_commands.py`）。
...
- `ToolCallChunk`（`:72`）——工具调用开始/进行中，带名称和参数预览，**由网关侧决定呈现方式**（emoji、截断、详细/紧凑、或在某些平台上干脆吞掉）
- `Commentary`（`:85`）——工具迭代之间模型说的完整过场话（"我先看看仓库结构"）——不是增量而是完整文本
...
**fresh-final 机制**（`gateway/config.py:575` 起）——如果流式响应持续超过一定时间（以 Telegram 默认 60 秒为例），最终版本作为新消息发送（而非 edit），让平台时间戳反映实际完成时间。
...
**优雅关闭**（`run.py:5836` 起）——`SIGTERM` 触发优雅关闭：停止接受新消息、等待正在运行的 Agent 完成（`restart_drain_timeout` 配置超时）、关闭所有平台连接、清理 Agent 缓存。`shutdown_forensics.py`（462 行）在关闭过程中记录每个 Agent 的状态，帮助排查关闭时的异常。
...
├── slash_commands.py    — 42 个斜杠命令 mixin（4,624 行，同上）
