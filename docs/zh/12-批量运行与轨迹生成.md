...
`toolset_distributions.py` 定义了 **17 种分布**（`DISTRIBUTIONS` 字典，`toolset_distributions.py:29-220`），每种是一个「主题化」的工具组合概率配置。典型几个（每个分布通常含多个工具集，下面只摘前两高概率项）：`default`（所有工具 100%）、`research`（web 90% + browser 70%，另含 vision 50/moa 40/terminal 10）、`development`（terminal/file 80% + moa 60% 等）、`browser_only`（只浏览器）、`safe`（无终端模式）。
