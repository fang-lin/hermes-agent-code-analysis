...
> **本章定位**：训练数据生成管线——`batch_runner.py`（1,321 行，批量轨迹生成）、`trajectory_compressor.py`（1,508 行，轨迹压缩）、`mini_swe_runner.py`（735 行，SWE 任务 runner）、`toolset_distributions.py`（364 行，工具集随机化）、`datagen-config-examples/`（配置模板）。
> **关键类/函数**：`BatchRunner`（`batch_runner.py:527`）、`CompressionConfig`（`trajectory_compressor.py:83`）、`compress_trajectory()`（`trajectory_compressor.py:709`）、`sample_toolsets_from_distribution()`（`toolset_distributions.py:247`）。
...
| `--model` | `claude-sonnet-4.6` | 用哪个模型（这是 CLI 入口默认值；用 Python API 直接构造 `BatchRunner()` 时类默认是 opus，注意区别） |
...
`toolset_distributions.py` 定义了 **17 种分布**（`DISTRIBUTIONS` 字典，`toolset_distributions.py:29-220`），每种是一个「主题化」的工具组合概率配置。典型几个：`default`（所有工具 100%）、`research`（web 90% + browser 70%）、`development`（terminal/file 80% + moa 60%）、`browser_only`（只浏览器）、`safe`（无终端模式）。
...
压缩策略（`compress_trajectory()`，`trajectory_compressor.py:709`）遵循一个清晰原则——**保头保尾压中间**：
...
└── compress_trajectory():709  — 保头保尾压中间

mini_swe_runner.py           — SWE runner（735 行，local/docker/modal）
toolset_distributions.py     — 17 种工具集分布 + 独立概率采样（364 行）
