...
```mermaid
flowchart TD
    CALLER["调用者"] -->|"run_conversation / interrupt / steer"| AGENT["AIAgent"]
    AGENT -->|Transport 层| LLM["LLM API"]
    AGENT -->|handle_function_call| TOOLS["72 个工具"]
    AGENT --> PERSIST["SQLite · 记忆 · 轨迹"]
```
...
```mermaid
flowchart LR
    INIT["__init__"] --> ACTIVE["活跃"]
    ACTIVE --> RUN["run_conversation × N"]
    RUN --> ACTIVE
    ACTIVE --> CLOSE["close#40;#41;"]
```
...
flowchart TD
    subgraph SYS ["① system message"]
        direction LR
        STABLE["stable 层"] --> CONTEXT["context 层"] --> VOLATILE["volatile 层"]
    end

    subgraph TOOLS_S ["② tools schema · 72 个工具 · ~20K+ token"]
        T1[" "]
    end

    subgraph MSGS ["③ messages"]
        direction LR
        H1["user"] --> H2["assistant + tool_call"] --> H3["tool result"] --> H4["assistant"] --> H5["user + 注入"]
    end
...
```mermaid
flowchart LR
    API["失败"] --> CLS["error_classifier"]
    CLS --> R429["429"] --> FIRST["等待重试"] --> SECOND["切换凭证"] --> ALLX["fallback"]
    CLS --> OTHER["其他错误 → 退避重试"]
```
