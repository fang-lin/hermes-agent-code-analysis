...
名称冲突规则：后加载的覆盖先加载的（同名同 kind）。但跨工具集的同名注册默认被拒绝（防意外遮蔽；MCP 对 MCP 覆盖例外），插件想替换内置工具实现须显式传 `override=True`（`registry.register` 签名 `registry.py:356`，另有按插件命名空间的 override 政策开关 `:307`）。
