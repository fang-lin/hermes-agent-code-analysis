...
plugins:
  enabled:
    - disk-cleanup
    # 注：spotify 是 backend 类插件（bundled 后端自动加载），
    # 写进 enabled 是冗余的、不写也会加载；此处仅作演示
...
> - [构建插件](https://hermes-agent.nousresearch.com/docs/developer-guide/plugins)
...
`PluginContext`（`plugins.py:337`）是 Agent 暴露给插件的 API 表面——插件能做的事情由它的方法列表严格界定。v0.18.2 共 22 个公开成员（20 个方法 + 2 个属性）。下表用 ★ 标出 v0.17-v0.18 新增的 3 个注册面（`register_secret_source`/`register_slack_action_handler`/`register_middleware`）；另有 3 个注册面（dashboard_auth/tts/transcription provider）虽也较新但早在 v0.15 就已加入，不再单独标注：
...
| `register_dashboard_auth_provider()` | 注册 Dashboard 认证方案（对接桌面/Web，→ 08/14 章；v0.15 起） | `plugins.py:673` |
...
| `register_tts_provider()` | 注册 TTS 引擎（v0.15 起） | `plugins.py:847` |
| `register_transcription_provider()` | 注册 STT 引擎（新 Python 引擎；6 个内置后端不走这里；v0.15 起） | `plugins.py:885` |
...
| `llm` property | 访问 `PluginLlm` facade（内部转调 `auxiliary_client`，带 per-plugin 信任门控） | `plugins.py:349` |
...
`llm` property 返回一个 `PluginLlm` facade（`agent/plugin_llm.py`），插件通过它访问 02 章讲过的辅助 LLM 客户端（`auxiliary_client.py`）做自己的推理，不占用主模型配额。这个 facade 有一套和"工具 override 需 `allow_tool_override` 许可"同源的安全设计——模型/provider/auth 覆写默认 fail-closed，必须经 `plugins.entries.<plugin_id>.llm.*` 配置显式放开，防止插件擅自改用别的模型或凭证。
...
**图：插件钩子在 Agent 工作流中的触发位置**（图中展示的是主对话循环内的钩子；不在主循环上的还有：`subagent_start`/`subagent_stop`（子 Agent 起止）、`pre_gateway_dispatch`（Gateway 授权检查前，见 05 章）、`api_request_error`（API 报错时）、`pre_verify`（代码编辑后的验证门控，可让 Agent 继续跑检查而非停下）、以及 kanban 三件套 `kanban_task_claimed/completed/blocked`（看板任务生命周期事件，定义于 `hermes_cli/plugins.py` 的 `VALID_HOOKS`），合计 23 种 `VALID_HOOKS`）
...
├── image_gen/        — 图像生成后端（6 个；krea v0.15 加入，openrouter v0.18 新增）
├── video_gen/        — 视频生成后端
