...
钩子只能**观察或拦截**；v0.18 新增的中间件（`hermes_cli/middleware.py`）能**改写载荷、包裹执行本身**。`register_middleware()` 支持四种 kind（`middleware.py:20-23`）：`tool_request` / `tool_execution` / `llm_request` / `llm_execution`，已接入 conversation_loop、tool_executor、model_tools 的生产链路。
