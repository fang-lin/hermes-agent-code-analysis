...
`inject_message()` 用于外部事件桥接——以 Google Meet 插件为例，会议中有人说话时，插件把转录文本注入 Agent 的对话流。注意：**Gateway 模式下 `inject_message()` 静默失败**（`_cli_ref` 为 None 时 `return False`，`plugins.py:373`），因为它依赖 CLI 的输入队列。Agent 正在执行任务时注入会中断当前任务；Agent 空闲时注入则进入队列，等待下一轮处理。
...
**图：插件钩子在 Agent 工作流中的触发位置**（图中展示的是主对话循环内的钩子；另有两个不在主循环上的钩子未画出——`subagent_stop`（子 Agent 结束时）和 `pre_gateway_dispatch`（Gateway 授权检查前，见 05 章），合计 17 种 `VALID_HOOKS`）

关键钩子说明：

- **`pre_tool_call`**：最特殊——可以返回 `{"action": "block", "message": "..."}` **阻断工具调用**。注意执行模型：`invoke_hook` 会无条件调用所有已注册的 `pre_tool_call` 回调（无提前退出），随后 `get_pre_tool_call_block_message` 取**第一个**有效的 block 消息作为阻断理由（后续 block 消息被忽略，但所有插件的回调都已执行过）。这让插件可以实现自定义安全策略（以速率限制插件为例）
...
**`platform`**——平台适配器插件（05 章提到的 `plugins/platforms/`），为 Gateway 提供新的消息平台支持。以 Discord 插件为例，它实现了 `BasePlatformAdapter` 接口。bundled 的 platform 插件自动加载。
