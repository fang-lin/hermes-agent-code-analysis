...
- **管理面板**——技能（浏览/安装，对接 04 章的多源 Hub）、cron 任务（可视化蓝图表单，对接 11 章）、gateway/消息平台状态（web_server 另有插件启停 API `post_agent_plugin_enable/disable`，但桌面前端当前未接入该功能）
- **文件浏览器与内嵌终端**——右侧栏项目树（tui_gateway 的 git_probe/project_tree RPC，第 10 章）；内嵌 xterm.js 终端由 Electron 主进程本地起 node-pty、经 contextBridge IPC 通信（与第 10 章 Web Dashboard 的 pty_bridge/WebSocket 是两套不同实现）
...
flowchart TD
    ...
    subgraph DESK ["apps/desktop"]
        MAIN["Electron 主进程<br/>main.cjs 7,716 行 + 33 个纯函数 .cjs 模块"]
...
### Electron 主进程：backend 保姆 + 33 个纯函数模块

安装器的活到这里就结束了——它拉起的完整桌面应用，此后启动、认证、更新、卸载的整条生命周期都归 Electron 主进程管。`main.cjs`（7,716 行）是这条生命周期的心脏，但这个"上帝文件"的周围是一圈**刻意与 Electron 解耦的纯函数模块**——`backend-command.cjs`、`backend-ready.cjs`、`backend-probes.cjs`、`connection-config.cjs`、`dashboard-token.cjs`、`bootstrap-runner.cjs`（739 行）……这些纯函数模块共 **33 个**（`electron/` 顶层 68 个 .cjs = `main.cjs` 1 个 + 33 个模块 + 34 个 `*.test.cjs` 测试文件；页首"68 个 .cjs"指的是含测试文件的总数）——多数配一个 `node --test` 的同名测试文件（少数例外：`embed-referer`/`git-repo-scan`/`preload` 无测试，另有几个测试文件测的是 `main.cjs` 内联逻辑），注释明说"kept standalone (no require('electron')) so it can be unit-tested"。主进程的可测试逻辑全部外置，`main.cjs` 只做胶水和 IPC 布线——这是对"Electron 主进程难以测试"这个行业通病的直接回答。
...
- **更新检测（只读）**：`checkUpdates()`（`main.cjs:1905`）只做只读比对——`git fetch` + SHA 比较 + `git status --porcelain`，供 UI 显示"落后 N 个 commit"，处理 shallow clone 无 merge-base 时退化为 SHA 比较（#51922）、脏工作树、SSH/HTTPS 远程差异。它本身不写任何东西
- **执行更新（`applyUpdates()`，`main.cjs:2222`）**：三条分支但**都要重启**——打包安装（有 staged updater）时 `app.quit()` 把控制权交给 Tauri updater 走下面的完整重装；POSIX 无 updater 时走 `applyUpdatesPosixInApp()` 自己 `hermes desktop --build-only` 重建再 relaunch；CLI 安装无 updater 时只给用户一条手动命令、桌面自己什么都不做。所谓"双轨"其实是同一套更新在不同平台的两种落地，没有"不重启就地增量更新"这条独立通道。Tauri 侧 `update.rs`（1,267 行）的完整重装四步走：
  1. 等旧桌面退出（最多 20 秒）
...
OAuth 会话本身靠两级 cookie 维持：`hermes_session_at`（Access Token，约 15 分钟）+ `hermes_session_rt`（Refresh Token，24 小时滚动）。"要不要尝试连接/显示为已登录"的活性判断用 `cookiesHaveLiveSession()`——**AT 或 RT 任一 cookie 存在即算活**（不是只认 RT）：AT 每 15 分钟过期是设计常态，只要 24 小时的 RT 还在，gateway 中间件会在下次请求时用 RT 静默换新 AT；如果只认 AT，用户就会每 15 分钟被迫重新登录一次。真正的权威活性校验是连接时铸 ws-ticket（RT 也失效时它会返回真正的 401）。与之相对的 `cookiesHaveSession()` 才是"只看 AT 未过期"的版本。
...
| right-sidebar | 5,503 | 右侧栏：项目树、git 状态（吃 tui_gateway 的 git_probe/project_tree RPC）；**内嵌 xterm.js 终端**也在此（`terminal/` 子目录 2,253 行，走 IPC 到主进程 node-pty）。注：桌面把会话按 **Projects**（具名工作区，per-profile `projects.db`，后端 `tools/project_tools.py` 的 `project` 工具集仅 GUI 会话可见）分组——这是桌面/TUI 独有、CLI/消息/cron 不带的子系统 |
| starmap | 3,817 | 会话星图可视化（Star Map / Memory Graph——同一份 `agent.learning_graph` 数据也喂给 CLI 的 `hermes journey` 命令和 `/journey` TUI 覆盖层，三端同源） |
| skills | 3,014 | 技能浏览/安装（对接多源 Hub，第 04 章） |
| shell | 2,942 | 应用外壳：标题栏/状态栏/模型菜单（非终端——终端在 right-sidebar/terminal） |
...
桌面对主仓的全部依赖是一条 HTTP/WebSocket API 加一个 stdout 就绪握手。Python 侧对"桌面"的感知被压缩成几个环境变量开关（`HERMES_DESKTOP=1` 等）——web_server 用它切行为，`read_terminal`/`close_terminal` 两个工具的可见性由它门控，`hermes_cli/main.py` 据它注入一组桌面专属环境变量。面窄且都是开关式的，没有任何 Python 模块 import 桌面代码或依赖桌面的内部结构。这让桌面可以独立发版（0.17.0 的桌面配 0.18.2 的后端），也让后端 API 被迫保持向后兼容——`serve`/`dashboard --no-open` 的双轨兼容就是这个约束的产物。

#### 主进程逻辑纯函数化

前文已述其形态（33 个无 electron 依赖的纯函数模块、测试几乎一比一）；作为决策，值得记的是它选的隔离策略。面对"Electron 主进程难测试"这个行业通病，另一条常见路是把主进程包成可依赖注入的类、测试时 mock 掉 electron——desktop 选了物理拆文件：被测逻辑根本不 `require('electron')`，测试就不需要任何 electron 运行时和 mock 基建，`node --test` 裸跑即可。代价是 `main.cjs` 只剩胶水和 IPC 布线也仍有 7,716 行。和 Python 侧 god-file 分解（第 00 章）选的是同一个方向：与其让巨文件可测，不如让可测的部分离开巨文件。
