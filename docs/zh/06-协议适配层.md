...
v0.17 新增 `acp_adapter/provenance.py`（127 行）解决一个编辑器视角的困惑：Hermes 的上下文压缩会**轮转内部会话 ID**（压缩后开新 head、旧会话作为 parent 链上去），但编辑器手里的 ACP `session_id` 是稳定的公共句柄——两边的 ID 体系会渐行渐远。
...
├── provenance.py     — 会话血统追踪（127 行，v0.17 新增）
├── auth.py           — 认证（79 行）
├── __init__.py       — 包入口（1 行）
...
安全考虑。MCP Serve 的目标是让外部 AI 读写消息，不是让它直接在你的机器上执行命令。如果暴露了 `terminal` 或 `write_file` 工具，外部 AI 就能绕过 hermes-agent 的安全审批机制直接操作文件系统。而 `messages_send` 只是复用 Gateway 的**出站投递面**——不触及 terminal/file 这类执行能力、也不触发 Agent 执行（与"安全边界"节一致），暴露它不会打开执行面的口子。
