...
- **三种写入形态都触发审批**（v0.18 行为变化）——`build_edit_proposal()`（`edit_approval.py:178-189`）为 `write_file`、`patch mode=replace`、以及 `patch mode="patch"`（V4A 多文件补丁格式）三者生成审批提案。V4A 在 v0.14 时代**不**触发审批，v0.18 补上了这个缺口：新增 `_proposal_for_patch_v4a()` 与 `_extract_v4a_patch_paths()`（`:131-160`）从 V4A 补丁文本中解析涉及的文件路径生成 diff 提案——多文件补丁不再是绕过编辑器审批的后门
- **60 秒超时等于拒绝**（`make_acp_edit_approval_requester` 的 `timeout` 默认值 60.0，`edit_approval.py:290`）——编辑器未在 60 秒内响应审批请求，文件写入被阻止

#### ACP 的工具集限制

ACP 使用专门的 `hermes-acp` 工具集（`toolsets.py`），它是核心工具集的**子集**——移除了 `clarify`（编辑器有自己的交互方式）和 `send_message`（不需要跨平台消息）。新增了 ACP 专有的编辑审批工具。会话建立时还会按需注入记忆 Provider 工具（`inject_memory_provider_tools()`，`server.py:831/847`）。
...
另外两个 v0.18 的编辑器体验细节：Agent 的工作目录跟随编辑器传来的 `session_cwd`（`session.py:336` 起）；用户中断时内部的中断提示消息**不再发给 ACP 客户端**——改用协议标准的 `stop_reason` 表达（`server.py:1312` 起），编辑器不会再看到一条莫名其妙的"[interrupted]"文本。
...
EventBridge 的关键设计细节：
- **索引已迁移 state.db**（v0.18，#8925）：会话路由索引 `_load_sessions_index()`（`mcp_serve.py:82`）现在**优先读 state.db**，`sessions.json` 降级为回退——旧的双文件读取有竞态（两份数据不同步时路由错会话）。轮询也只查 state.db 的 mtime（`:355` 附近），文件没变就不碰数据库——使 200ms 轮询几乎无开销
