...
> **关键目录**：`plugins/memory/`（8 个记忆 Provider，12,054 行）、`plugins/model-providers/`（30 个 Provider，1,199 行）、`plugins/platforms/`（7 个消息平台，15,326 行）。
...
07 章建立了插件的框架——PluginContext API、18 种钩子、五种 kind 类型。但框架本身不产生价值，使用框架的插件才产生价值。
...
#### memory（8 个记忆 Provider，12,054 行）
...
| Provider | 核心能力 | 行数 |
|----------|---------|------|
| honcho | 跨会话用户建模、辩证 Q&A、语义搜索、持久结论 | ~5,000 |
| hindsight | Hindsight 客户端集成 | ~600 |
| holographic | 全息记忆（向量 + 图谱） | ~800 |
| mem0 | Mem0 记忆 API | ~700 |
| openviking | OpenViking 记忆后端 | ~400 |
| retaindb | RetainDB 本地记忆 | ~500 |
| supermemory | SuperMemory 集成 | ~600 |
| byterover | ByteRover 记忆后端 | ~400 |
...
| 平台 | 行数 | 特点 |
|------|------|------|
| Discord | ~5,000 | 语音模式、斜杠命令、角色授权、线程、频道技能绑定 |
| Google Chat | ~2,000 | Google Workspace 集成 |
| Teams | ~2,000 | Microsoft Teams 集成 |
| IRC | ~1,500 | 传统 IRC 协议 |
| Line | ~1,500 | LINE 消息平台 |
...
通过 `ctx.register_browser_provider()` 注册云浏览器服务（以 Browser Use 为例），让 Agent 的浏览器工具可以使用远程浏览器而非本地 Playwright。
...
`plugins/google_meet/` 实现了 Google Meet 会议转录集成。v1 版本通过 Playwright 爬取 Meet 的 live captions 到 transcript 文件，Agent 通过 `meet_transcript` 工具读取该文件——不是实时注入，而是文件中转模式。插件注册了多个工具（`meet_start`、`meet_stop`、`meet_transcript` 等），让 Agent 能控制转录的开始/停止、读取转录内容、生成会议摘要。
...
每个 model-provider 插件平均只有约 40 行——因为它们只声明元数据（Provider 名称、认证方式、API Key 环境变量），不实现任何逻辑。实际的认证流程、API 调用、Transport 适配全部由核心系统（`auth.py`、`runtime_provider.py`、`agent/transports/`）统一处理。这是"声明式插件"的典型模式——插件只说"我是谁"，框架负责"怎么用"。
...
| 09 — Kanban 系统 | kanban 插件是 Kanban 系统的 Agent 侧调度器 |
