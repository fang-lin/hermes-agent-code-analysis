...
hermes skills browse install <name>  # 从 Hub 安装技能
```
...
**场景一：安装社区技能。** `hermes skills browse` 浏览 agentskills.io，找到想要的技能后 `hermes skills browse install <name>`。安装的技能放在 `~/.hermes/skills/` 下，和内置技能使用方式相同。

**场景二：Agent 自动创建技能。** 不需要你做任何事——当 Agent 在一次对话中使用了较多工具调用（说明任务复杂），对话结束后 `background_review.py` 会在后台 fork 一个 review Agent，用辅助模型审查这次对话，如果发现可复用的模式就自动创建技能。
...
| Agent 不自动创建技能 | 检查 `skills.auto_improve` 是否为 true；`skills.creation_nudge_interval` 是否合理（默认 10 次工具迭代）；确认 `skill_manage` 在当前工具集中 |
...
改进不需要人工触发——模型在执行技能时如果发现问题，会主动修改。`skills.auto_improve: true`（默认开启）控制这个行为。
...
hermes skills browse search "finance"     # 搜索
hermes skills browse install stock-trader # 安装
hermes skills browse publish my-skill     # 发布自己的技能
```
...
2. **Skills Hub 安装**：`hermes skills browse install <name>`
...
可选技能不随 hermes-agent 默认安装，需要通过 `hermes skills browse install` 或手动放入 `~/.hermes/skills/` 启用。
