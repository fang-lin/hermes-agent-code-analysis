...
| 问题 | 排查方向 |
|------|---------|
| 模型不调用某个工具 | `hermes tools` 确认工具已启用；检查工具的 `check_fn` 是否返回 True（以 Browser 为例，需要 Playwright）；确认工具在当前平台的 toolset 中 |
| 工具调用报错 | 检查 `~/.hermes/agent.log`；`handle_function_call()`（`model_tools.py:741`）是调度入口，错误会被捕获并返回 JSON error |
| 命令被拒绝（"denied"） | 检查是否触发了 `HARDLINE_PATTERNS`（无条件拒绝，即使 --yolo 也不行）；检查 `DANGEROUS_PATTERNS`（`approval.py:316`，15 个正则）；查看 `config.yaml` 的 `command_allowlist` |
...
%%{init: {"theme": "neutral", "themeVariables": {"fontSize": "14px"}, "flowchart": {"nodeSpacing": 15, "rankSpacing": 25}}}%%
flowchart TD
    TC["LLM 返回 tool_calls"] --> PAR{{"可以并行？"}}
    PAR -->|只读工具| CONC["ThreadPoolExecutor 并行执行"]
    PAR -->|有副作用| SEQ["逐个串行执行"]
    CONC --> HFC["handle_function_call#40;#41;"]
    SEQ --> HFC
    HFC --> LOOKUP["ToolRegistry 查找 handler"]
    LOOKUP --> APPROVE["安全检查<br/>approval · path · URL · Tirith"]
    APPROVE --> EXEC["执行 handler"]
    EXEC --> SIZE["结果大小治理<br/>超 100K → 持久化"]
    SIZE --> HOOK["transform_tool_result 钩子"]
    HOOK --> BACK["结果回填到 messages"]
```

**图：工具调用的完整执行路径——从并行判断到安全检查到结果回填**

关键步骤说明：

1. **并行判断**（`agent/tool_dispatch_helpers.py:103`）：`_should_parallelize_tool_batch()` 检查这批工具调用是否可以并行——只读工具（以 `read_file`、`web_search` 为例）可以并行；有副作用的工具串行执行。完全串行太慢，完全并行可能有竞态——按副作用分类是务实的折中
2. **handle_function_call()**（`model_tools.py:741`）：调度入口。从 `ToolRegistry` 查找对应的 handler，捕获所有异常转为 JSON error
3. **安全检查**：在执行 handler 之前，terminal 工具的命令会经过 `check_all_command_guards()`（详见下文"安全三道防线"）
4. **结果大小治理**：handler 返回后，超大结果被持久化（详见下文"结果大小治理"）
5. **插件钩子**：`transform_tool_result` 钩子允许插件修改工具结果（以日志观测性插件为例，记录每次工具调用的耗时和结果摘要）
...
3. **--yolo / approvals.mode=off**——用户显式选择跳过所有审批（除 HARDLINE 外）
...
安全方面：MCP 子进程只继承白名单环境变量（PATH、HOME、USER 等），敏感凭证不泄露到外部进程。
