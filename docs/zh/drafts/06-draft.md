...
**例一：你想让 Agent 能查股票价格。** 这用技能就够了——写一个 SKILL.md 描述"用户问股价时执行 scripts/stock.py"，脚本通过 `execute_code` 在沙箱里调用 API 返回数据。技能给 Agent 增加了一个新的"做事能力"。

**例二：你想让每次 LLM 调用的 token 用量和延迟都发送到 Langfuse 做监控。** 技能做不了这件事——因为技能是被 Agent 调用的外部脚本，它不知道 LLM 调用何时发生，也没有办法在 LLM 调用前后插入自己的代码。你需要的是在 Agent 内部的 `post_api_request` 节点注册一个回调——这只有插件能做。

两者的根本区别不是"能不能跑 Python"——技能也可以包含 Python 工具脚本。区别在于**谁调用谁**：

| | 技能 | 插件 |
|---|------|------|
| 执行方式 | Agent 调用技能脚本（沙箱） | 插件代码加载到 Agent 进程内 |
| 能做什么 | 给 Agent 增加"做事"能力 | 介入 Agent 的内部工作流 |
| 典型场景 | 查股价、写报告、分析数据 | 监控 LLM 调用、替换记忆系统、拦截危险操作、对接 Spotify |
| 控制粒度 | 模型决定何时调用 | 在 18 个生命周期节点自动触发 |
| 能否拦截 | 不能——脚本不知道其他工具在做什么 | 可以——`pre_tool_call` 能阻断任何工具调用 |
| 能否替换组件 | 不能——脚本在沙箱中，接触不到 Agent 内部 | 可以——替换记忆 Provider、上下文引擎、图像生成后端 |

**什么时候用技能？** 当你想让 Agent 能"做一件新的事"——查数据、生成报告、操作 API——用技能。它简单、安全（沙箱隔离）、模型按需调用。

**什么时候用插件？** 当你想改变 Agent "怎么工作"——监控每次调用、在工具执行前审查、替换底层的记忆或压缩机制、把新的消息平台接入 Gateway——用插件。它强大但需要理解 Agent 内部结构。

**为什么不把两者合并？** 因为安全和复杂度的权衡。技能脚本在沙箱中执行，即使有 bug 也不会破坏 Agent。插件在进程内运行，一个写错的 `pre_tool_call` 钩子可以阻断所有工具调用。把两者合并意味着所有技能都获得了进程内访问权——这对安全模型来说是不可接受的。
