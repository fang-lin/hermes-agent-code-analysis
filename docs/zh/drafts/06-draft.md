...
技能（第 03 章提到的 `skills/`）是自然语言层面的扩展——它们通过 SKILL.md 告诉模型"遇到这种问题用这种方法"。但技能有一个根本限制：它只能引导模型的行为，不能改变 Agent 本身的能力。如果你想让 Hermes 对接 Spotify 播放音乐、把 LLM trace 发到 Langfuse、或者替换默认的上下文压缩策略——这些需要**运行时的 Python 代码**。
