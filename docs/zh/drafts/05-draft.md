...
| MCP 工具无响应 | 最常见原因：`mcp` Python 包未安装（`pip install mcp`），导致 `_MCP_SERVER_AVAILABLE = False`（`mcp_serve.py:50`）。其次确认 Gateway 正在运行（MCP serve 依赖 SessionDB） |
...
flowchart LR
    ...
    ACP --> SM["SessionManager<br/>session.py:628"]
...
`HermesACPAgent`（`server.py:445`）继承自 `acp.Agent`，是 ACP 协议的服务端实现。它管理：
...
- **ContextVar 隔离**（`server.py:1501`）：每个 `_run_agent` 调用运行在 `contextvars.copy_context().run()` 内，确保会话级 ContextVar（`HERMES_SESSION_KEY`、`ACP_EDIT_APPROVAL_REQUESTER`）不会在共享线程间泄漏
...
- **敏感路径始终询问**（`edit_approval.py:44`）：`{".env", ".env.local", "id_rsa", "id_ed25519"}` 以及 `.git/`、`.ssh/` 目录下的文件，**即使在 `dont_ask` 模式下也不会自动批准**。这是安全底线
...
注意区分：第 03 章讲的 MCP 是 hermes-agent 作为 MCP **客户端**接入外部工具；这里的 MCP Serve 是 hermes-agent 作为 MCP **服务端**暴露自己的能力。方向相反。
...
10 个 MCP 工具的设计兼容了 OpenClaw 的 9 工具 MCP 通道桥接接口（`mcp_serve.py:9`），加上 Hermes 特有的 `channels_list`——这意味着从 OpenClaw 迁移过来的用户不需要修改 MCP 客户端配置。
