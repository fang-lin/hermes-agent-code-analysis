...
Each platform has a special behavior. On macOS the Hermes icon in /Applications is a dual identity (`lib.rs:116-154`): both installer and launcher — detecting it's already installed, it skips the install interface and quickly launches the desktop app directly. On the Windows side even where the `windows_subsystem = "windows"` declaration should go — in `main.rs` — has a comment recording the lesson: putting it wrongly in lib.rs pops up a spurious cmd black window.
...
1. **Launch** (`backend-command.cjs`): spawn `hermes serve --host 127.0.0.1 --port 0` — `--port 0` lets the OS assign a random port. There's a piece of compatibility logic against "an upgrade half-bricking it": `serve` is a newer subcommand (registered at `hermes_cli/subcommands/dashboard.py:137`), and if the resolved runtime doesn't recognize it yet, after statically probing its source it falls back to the equivalent `dashboard --no-open`. Before launch there are two gates:
   ...
   - **A failure latch** (`startHermes()`'s `bootstrapFailure`/`backendStartFailure`) — a failure once re-throws the same error, preventing renderer retries from dragging the user into a repeated-install loop.
