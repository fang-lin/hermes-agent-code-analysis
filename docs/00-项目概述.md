...
graph TD
    ...
    CLI --> Agent["Agent 核心<br/>run_agent.py (AIAgent)<br/>+ agent/ 子模块<br/>(Transport / 记忆 / 压缩 / 提示词)"]
    GW --> Agent
    Agent --> Tools["工具层<br/>tools/ (66 个文件)<br/>model_tools.py + toolsets.py"]
    Tools --> Skills["技能系统<br/>skills/"]
    Tools --> Plugins["插件系统<br/>plugins/"]
    Tools --> Cron["定时调度<br/>cron/"]
    Bridge["外部桥接<br/>mcp_serve.py (MCP 服务端)<br/>acp_adapter/ (ACP 协议适配)"]
```
