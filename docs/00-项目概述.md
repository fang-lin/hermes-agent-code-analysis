...
这些模块的依赖关系是**严格单向的**：CLI/网关 → Agent 核心 → 工具层。没有反向依赖（需要反向调用的地方，比如工具层里的子代理需要创建新的 AIAgent，都用 lazy import 打破循环）。
...
Hermes 的应对方案是一个**上下文压缩器** (`agent/context_compressor.py`)。当对话历史占满 75% 的上下文窗口时（`agent/context_engine.py:59`），它会用一个廉价的辅助模型把中间的对话摘要化，只保留头部几条（保持系统提示稳定）和尾部几条（保持最近上下文）。这不是简单截断——它是用 LLM 做有损压缩，试图在保留关键信息的同时腾出空间。
...
**核心依赖极其克制** (`pyproject.toml:13-37`)。基础安装只有十几个包：`openai` 和 `anthropic` 两个模型 SDK，`pydantic` 做数据校验，`prompt_toolkit` 做终端 TUI，`rich` 做富文本渲染，`jinja2` 做提示词模板，`httpx` 做 HTTP 客户端。所有平台集成、语音、MCP、RL 训练都通过 optional extras 按需安装 (`pyproject.toml:39-126`)——这就是为什么它能跑在 5 美元的 VPS 上，也能跑在 GPU 集群上。

**前端用 Node.js** (`package.json`)——Web Dashboard 和 TUI 都是 npm 打包的前端应用。浏览器自动化也走 Node.js（Playwright + CamoFox）。

**Docker 构建两阶段** (`Dockerfile:1-67`)——用 `uv`（Rust 写的 Python 包管理器）代替 pip，安装速度快一个数量级。运行时用 `tini` 做 PID 1 进程管理，防止僵尸进程积累（`Dockerfile:14` 的注释解释了为什么——MCP stdio 子进程和 git 子进程容易变僵尸）。
...
Hermes 有三个主要入口点，但它们最终都调用同一个核心——`AIAgent.run_conversation()` (`run_agent.py:9627`)：
...
**`mcp_serve.py`** 把 Hermes 的会话能力暴露为 MCP 工具（`mcp_serve.py:8-13`），让 Claude Code、Cursor 这样的 MCP 客户端可以直接调用 Hermes 的对话、记忆和平台投递功能。
...
阅读这个代码库时，一个自然的问题是：578,000 行 Python，9 个月内 6,384 次提交——这些代码有多少是人写的，多少是 AI 生成的？
...
- **这本身就是项目理念的体现。** 一个"自改进的 AI agent"项目，用 AI 来开发自己——这不是偷懒，而是 dogfooding。
...
这篇概述回答了"Hermes 是什么、有哪些模块、为什么这样设计、谁写的"。接下来的架构分析会深入"它具体怎么工作"——从一条用户消息进入系统开始，追踪它经历的每一层处理，理解每个设计选择背后的 tradeoff。
