...
graph LR
    ...
    subgraph F2["面孔 2: React/Ink TUI\n更丰富的 UI，但多一层进程间通信"]
        UITUI["ui-tui/ (Node.js)"] -->|JSON-RPC| TUIGW["tui_gateway/"] --> AG2["AIAgent"]
    end
    subgraph F3["面孔 3: Web Dashboard\n浏览器访问，最灵活但链路最长"]
        WEB["web/ (React SPA)"] --> FASTAPI["FastAPI"] --> PTY["PTY"] --> INKTUI["Ink TUI"] --> AG3["AIAgent"]
    end
```
