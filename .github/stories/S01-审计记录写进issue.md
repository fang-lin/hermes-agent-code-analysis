...
- [ ] **② 评估+规划**:往 issue 写一条标准记录(时间 / 触发 / 评估了哪些章 + 有没有覆盖缺口 / 复杂度 / 置信度 / 下一步),外加一个折叠块放**完整的 work plan**(要改哪几处、现状、改成什么、源码依据)。不再是干巴巴一行。
- [ ] **③ 同步**:往 issue 写标准记录,外加两个折叠块——一个放**每个复核 agent 的评语全文**(逐条核了什么、判过还是打回、举了哪段代码),一个放**本次逐处改动**(哪个文件、从什么改成什么,带 work plan 条目编号)。
- [ ] **复核循环**:同上,往 audit issue 写标准记录 + factual-reviewer 的评语全文 + 改了哪些。
- [ ] **token 用量**:每个用 agent 的层用 `claude -p --output-format json` 拿到 `total_cost_usd` / token 数,记进标准记录的"本层 + 累计"。纯脚本层记 0。
- [ ] **RUN_URL 接上**:标准记录标题带这次运行的 Actions 链接(`format_record` 的第二个参数),方便从 issue 跳到原始日志。
- [ ] 折叠块用 `format_details`,标准记录用 `format_record`(两个函数都现成)。
- [ ] 加一个针对"记录真的写进 issue"的验证(可在 S02 的真链路 smoke 里一并核:issue 里能看到标准记录 + 两个折叠块)。
